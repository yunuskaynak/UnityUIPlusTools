...
public class CustomHorizontalLayout : MonoBehaviour
{
    ...
    private void Start()
    {
        // Use a local function to safely run async code in Start
        RunStartup().Forget();
        return;
        ...
        async UniTaskVoid RunStartup()
        {
            ...
            {
                Debug.LogError($"CustomHorizontalLayout Start Exception: {ex}");
            }
        }
    }
    ...
    private void InitChilds()
    {
        ...
        foreach (Transform t in transform)
        {
            ...
            if (rect) _layoutChilds.Add(rect);
        }
    }
    ...
    private void InitSlotPositions()
    {
        ...
        foreach (var rect in _layoutChilds)
            _slotPositions.Add(rect.anchoredPosition);
    }
    ...
    public void FillSpacesToLeft() => EnqueueAnimation(FillSpacesToLeftInternal);
    ...
    private void EnqueueAnimation(Func<UniTask> anim)
    {
        if (_isLocked)
        {
            // Debug.LogWarning("CustomHorizontalLayout: Animation already in progress, ignoring new request.");
            return;
        }
    ...
    private async UniTaskVoid RunAnimationQueue()
    {
        ...
        {
            Debug.LogError($"CustomHorizontalLayout Animation Exception: {ex}");
        }
        _isLocked = false;
        _isAnimating = false;
    }
    ...
    private async UniTask ActivateChildInternal(RectTransform[] childRects, bool allAtOnce)
    {
        ...
        foreach (var rect in _layoutChilds)
        {
            ...
            if (rect.gameObject.activeSelf && Array.IndexOf(childRects, rect) < 0 && rect.anchoredPosition != _slotPositions[targetIdx])
                actives.Add(rect);
        }
        ...
        if (allAtOnce)
        {
            ...
            foreach (var rect in actives)
            {
                ...
                moves.Add(AnimateMoveAndStretch(rect, _slotPositions[idx]));
            }
        ...
        else
        {
            ...
            for (int i = actives.Count - 1; i >= 0; i--)
            {
                var rect = actives[i];
                int idx = _layoutChilds.IndexOf(rect);
                await AnimateMoveAndStretch(rect, _slotPositions[idx]);
            }
        }

        // 2. Activate all given (in call) objects together (scale-in in parallel, but only if not already active)
        ...
        foreach (var rect in childRects)
        {
            ...
            {
                rect.anchoredPosition = _slotPositions[idx];
    ...
    private async UniTask DeactivateChildInternal(RectTransform[] childRects, bool allAtOnce)
    {
        ...
        // 2. Fill spaces to the left (all at once or one by one)
        ...
        if (allAtOnce)
        {
            ...
            for (int i = 0; i < activeRects.Count; i++)
                if (activeRects[i].anchoredPosition != _slotPositions[i])
                    moves.Add(AnimateMoveAndStretch(activeRects[i], _slotPositions[i]));
            await UniTask.WhenAll(moves);
        }
        else
        {
            for (int i = 0; i < activeRects.Count; i++)
                if (activeRects[i].anchoredPosition != _slotPositions[i])
                    await AnimateMoveAndStretch(activeRects[i], _slotPositions[i]);
        }
    }
    ...
    private async UniTask SetToOriginalPositionsInternal()
    {
        for (int i = 0; i < _layoutChilds.Count; i++)
        {
            ...
            if (rect && rect.gameObject.activeSelf && rect.anchoredPosition != _slotPositions[i])
            {
                await AnimateMoveAndStretch(rect, _slotPositions[i]);
            }
        }
    }

    private async UniTask FillSpacesToLeftInternal()
    {
        int slotIndex = 0;
        foreach (var rect in _layoutChilds)
        {
            if (rect.gameObject.activeSelf)
            {
                if (rect.anchoredPosition != _slotPositions[slotIndex])
                {
                    await AnimateMoveAndStretch(rect, _slotPositions[slotIndex]);
                }
                slotIndex++;
            }
        }
    ...
    private async UniTask AnimateMoveAndStretch(RectTransform rect, Vector2 to)
    {
        if (rect.anchoredPosition == to)
            return;
        await rect.DOScale(Vector3.one * moveScale, moveScaleDuration).SetEase(animationEase).AsyncWaitForCompletion().AsUniTask();
        await rect.DOAnchorPos(to, moveDuration).SetEase(animationEase).AsyncWaitForCompletion().AsUniTask();
